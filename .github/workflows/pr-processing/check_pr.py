...
NA_TICKET = object()  # sentinel: ticket field is "N/A - typo" (typo-only PR)
...
def check_trac_ticket(pr_body: str, pr_files: list[str]) -> str | None | object:
    ...
    "N/A - typo" is also accepted for typo-only PRs.
    """
    ...
    if re.match(r"^N/A\s*-\s*typo$", cleaned, re.IGNORECASE):
        return NA_TICKET  # typo-only PR — skip Trac checks but don't fail
...
def main() -> None:
    ...
    if ticket_result is None:
        ...
        if ticket_result is NA_TICKET:
            print("N/A - typo ticket — skipping Trac status and has_patch checks.")
